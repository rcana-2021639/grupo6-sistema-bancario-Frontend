    ...
    const loadDashboard = async () => {
      try {
        ...
        try {
          ...
          setTransactions([]);
          setNotice('Las últimas transacciones no están disponibles para este rol.');
        }
      } catch (error) {
        setNotice(error.message || 'No se pudo cargar la información del dashboard.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-[#0066cc]">Dashboard</p>
          <h1 className="mt-1 text-2xl font-bold text-[#1e3a5f] sm:text-3xl">Hola, {userName}</h1>
          <p className="mt-2 text-sm text-slate-600">Resumen financiero de cuentas, saldos y movimientos recientes.</p>
        </div>
      ...
      {notice && (
        <div className="rounded-lg border border-blue-100 bg-blue-50 p-4 text-sm text-[#1e3a5f]">{notice}</div>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <StatCard label="Últimas transacciones" value={loading ? '...' : transactions.length} detail="Movimientos recientes" />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="rounded-lg border border-slate-200 bg-white lg:col-span-2">
          <div className="border-b border-slate-200 px-5 py-4">
            <h2 className="font-semibold text-[#1e3a5f]">Últimas transacciones</h2>
          </div>
          <div className="divide-y divide-slate-100">
            {loading ? (
              ...
              transactions.map((transaction) => (
                <div key={transaction._id || transaction.id} className="p-5">
                  ...
                  <p className="mt-2 text-sm text-slate-500">{transaction.description || 'Sin descripción'}</p>
                </div>
              ))
            )}
          </div>
